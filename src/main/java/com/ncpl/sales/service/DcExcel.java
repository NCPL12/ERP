...
// To read the message source from property file
	...
	protected static void buildExcelDocument(DeliveryChallan dc,SalesService salesService, String filePath, Map<String, String> map) throws Exception {
	// TODO Auto-generated method stub
			Workbook  workbook = new XSSFWorkbook();
	        Sheet editAccountSheet = workbook.createSheet("Dc Items");
			...
			createdDateCell.setCellValue("Unit");
			...
			populateSalesRecords(dcItemList,salesService, editAccountSheet, workbook,itemsList);
		...
		private static void populateSalesRecords(List<DeliveryChallanItems> dcItemList,SalesService salesService, Sheet editAccountSheet, Workbook workbook, Map itemsList) {
			int rowCount =1;
			...
			for (DeliveryChallanItems dcItem : dcItemList) {
				if(dcItem.getTodaysQty()!=0) {
					...
					String salesItemId=dcItem.getDescription();
					Optional<SalesItem> soItem=salesService.getSalesItemObjById(salesItemId);
					
				String description=soItem.get().getDescription();
				String unit=soItem.get().getItem_units().getName();
				float qty=dcItem.getTodaysQty();
				String slNo=dcItem.getSlNo();
				...
				threeSideborderRight.setWrapText(true);
				...
				threeSideborderRight.setBorderTop(BORDER_THIN);
				...
				poNoCell.setCellValue(designitems);
				
				Cell crDtCell = row.createCell(8);
				crDtCell.setCellStyle(threeSideborder);
				crDtCell.setCellValue(unit);
				
				Cell qtCell = row.createCell(9);
				qtCell.setCellStyle(threeSideborderRight);
