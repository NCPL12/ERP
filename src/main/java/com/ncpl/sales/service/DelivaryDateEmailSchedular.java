...
@Service
@EnableScheduling
public class DelivaryDateEmailSchedular {
	@Autowired
	...
	public void delivaryDateScheduler() throws IOException {
		...
		purchaseItemList = purchaseService.getPurchaseItemsWhereDlDateNotNullAndGrnNotDone();
		if(purchaseItemList.size()!=0) {
	    	try {
				DailyPurchaseExcel.buildExcelDocument(purchaseItemList, filePath,itemMasterService);
			} catch (Exception e) {
	...
	public Map<String, Object> purchaseInfoEmail() {
		...
		emailContents.put("subject","Delivery Date Info Report"); 
		emailContents.put("template","dl_date_info.html"); 
