		...
		Map itemsList = (Map) request.getAttribute("map");
		 ...
		 Party billAddress = (Party) request.getAttribute("billAddress");
		   if(billAddress!=null) {
	        ...
	        generateDeliveryChallanSheet(workbook, originalSheet, dcObject, request, user, itemsList, option,poNum,dcCurrentDate,items,response);

	        // Generate Sheet 2 (Duplicate)
	        Sheet duplicateSheet = workbook.createSheet("DeliveryChallan-Duplicate");
	        String option2="Duplicate";
	        generateDeliveryChallanSheet(workbook, duplicateSheet, dcObject, request, user, itemsList, option2,poNum,dcCurrentDate,items,response);
	        
	        Sheet triplicateSheet = workbook.createSheet("DeliveryChallan-Triplicate");
	        String option3="Triplicate";
	        generateDeliveryChallanSheet(workbook, triplicateSheet, dcObject, request, user, itemsList, option3,poNum,dcCurrentDate,items,response);
		
		
		
	}
	private void generateDeliveryChallanSheet(Workbook workbook, Sheet editAccountSheet,
			Optional<DeliveryChallan> dcObject, HttpServletRequest request, User user, Map itemsList, String option, String poNum, String dcCurrentDate, List<DeliveryChallanItems> items, HttpServletResponse response) throws IOException {
