...
@Service
@EnableScheduling
public class LrNumberEmailSchedular {
	@Autowired
	...
	public void delivaryDateScheduler() throws IOException {
		...
		purchaseItemList = purchaseService.getPurchaseItemsWhereLrNumEMpty();
		if(purchaseItemList.size()!=0) {
	    	try {
				DailyLrNoPurchaseExcel.buildExcelDocument(purchaseItemList, filePath,itemMasterService);
			} catch (Exception e) {
	...
	public Map<String, Object> purchaseInfoEmail() {
		...
		emailContents.put("subject","LR number Info Report"); 
		emailContents.put("template","lr_no_info.html"); 
...
class DailyLrNoPurchaseExcel{
	...
	public static void buildExcelDocument(List<PurchaseItem> purchaseItemList, String filePath, ItemMasterService itemMasterService) throws IOException {
        Workbook workbook = new XSSFWorkbook();
        Sheet editAccountSheet = workbook.createSheet("Purchase LR No. Report");
	...
	private static void populatePurchaseRecords(List<PurchaseItem> purchaseItemList, Sheet editAccountSheet, Workbook workbook,
			ItemMasterService itemMasterService) {
		int rowCount =1;
		int slNumber = 1;
		for (PurchaseItem purchaseItem : purchaseItemList) {
			...
			String poNum = purchaseItem.getPurchaseOrder().getPoNumber();
			Date created = purchaseItem.getPurchaseOrder().getCreated();
			...
			String description = purchaseItem.getPoDescription();
			Date dlDate = purchaseItem.getDelivaryDate();
			
			String dateString2 = dlDate.toString();
			...
			String vendorName = purchaseItem.getPurchaseOrder().getParty().getPartyName();
			...
			dlDtCell.setCellValue(formattedDate2);
