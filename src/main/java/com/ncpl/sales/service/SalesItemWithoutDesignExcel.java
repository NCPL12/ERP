...
@Service
@EnableScheduling
public class SalesItemWithoutDesignExcel {
	@Autowired
	...
	public void delivaryDateScheduler() throws IOException {
		...
		salesItemsList = salesService.getAllSalesItemListWithoutDesign();
		if(salesItemsList.size()!=0) {
	    	try {
	    		SalesItemWithNoDesignExcel.buildExcelDocument(salesItemsList, filePath,itemMasterService,salesService);
			} catch (Exception e) {
	...
	public Map<String, Object> purchaseInfoEmail() {
		...
		emailContents.put("subject","Sales Items Without Design"); 
...
class SalesItemWithNoDesignExcel{
	...
	private static void populateSalesRecords(List<SalesItem> salesItemsList, Sheet editAccountSheet, Workbook workbook,
		...
		for (SalesItem salesItem : salesItemsList) {
			...
			dlDtCell.setCellValue(salesItem.getItem_units().getName());
