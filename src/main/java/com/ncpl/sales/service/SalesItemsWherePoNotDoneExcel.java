...
@Service
@EnableScheduling
public class SalesItemsWherePoNotDoneExcel {
	@Autowired
	...
	public void delivaryDateScheduler() throws IOException {
		...
		salesItemsList = salesService.getAllSalesItemListWithouPO();
		if(salesItemsList.size()!=0) {
	    	try {
	    		SalesItemWithNoPOExcel.buildExcelDocument(salesItemsList, filePath,itemMasterService,salesService);
			} catch (Exception e) {
