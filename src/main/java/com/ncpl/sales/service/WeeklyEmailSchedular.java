...
@Service
@EnableScheduling
public class WeeklyEmailSchedular {
	@Autowired
	...
	public void delivaryDateScheduler() throws IOException {
		...
		purchaseItemList = purchaseService.getPurchaseItemsWhereDlDateForNextWeek();
		Collections.sort(purchaseItemList);
		if(purchaseItemList.size()!=0) {
	    	try {
	    		WeeklyDlDatePurchaseExcel.buildExcelDocument(purchaseItemList, filePath,itemMasterService,salesService);
			} catch (Exception e) {
...
class WeeklyDlDatePurchaseExcel{
	...
	public static void buildExcelDocument(List<PurchaseItem> purchaseItemList, String filePath, ItemMasterService itemMasterService, SalesService salesService) throws IOException {
		...
		dlDateCell.setCellValue("Del Date");
		...
		populateSalesRecords(purchaseItemList, editAccountSheet, workbook,itemMasterService,salesService);
	...
	private static void populateSalesRecords(List<PurchaseItem> purchaseItemList, Sheet editAccountSheet, Workbook workbook,
		...
		for (PurchaseItem purchaseItem : purchaseItemList) {
			...
			vendorCell.setCellValue(salesItemObj.get().getSalesOrder().getParty().getPartyName());
